...
<form method="post" action="add_information.jsp?display_count=5&page_tmp=1">
<table>
...
try {
	...
	if (test_connection == null){
		...
		//here movies.director =director && movies.title =title && movies.year = years;
...
// 		if(movie_id != 0 && !title.equals("") && request.getParameter("year") != null && !director.equals("") && (request.getParameter("star_firstname") !=null && request.getParameter("star_lastname") !=null )){
		...
		ResultSet flipping = select_movies.executeQuery("select id from movies where movies.director = '" +director + "' && movies.title= '" + title + "' && movies.year = " +year );
		while(flipping.next()){
			movie_id = flipping.getInt(1);	
			update_movie_check++;
				
		}
	 	String movie_exists = "movie_exists";
	 	if(update_movie_check == 0 ){
	 		movie_exists = "movie DOES NOT exists currently";
	 	}
		System.out.println(movie_exists);
		...
		System.out.println("THis is genre being entered" + genre);
		...
		error_printing.println(movie_exists);
